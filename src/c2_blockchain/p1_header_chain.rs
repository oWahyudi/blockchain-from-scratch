...
impl Header {
    ...
    fn verify_sub_chain(&self, chain: &[Header]) -> bool {
        ...
        for header in chain {
            if header.parent != hash(current_header) || header.height != current_header.height + 1 {
                return false;
            }
...
fn build_valid_chain_length_5() -> Vec<Header> {
    ...
    for _ in 1..=5 {
        let child = current_header.child();
...
fn build_an_invalid_chain() -> Vec<Header> {
    ...
    //add invalid child
    let mut invalid_child = current_header.child();
    invalid_child.height += 2;
    new_chain.push(invalid_child);
